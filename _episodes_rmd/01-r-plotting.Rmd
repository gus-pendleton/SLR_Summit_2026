...
cat("\"/Users/augustuspendleton/Desktop/nerr_data\"")
```
...
read_csv(file = 'data/water_quality.csv')
```
...
>  read_csv(file = '../data/water_quality.csv') # reads in csv file
> ```
{: .callout}
...
water_quality_oliver <- read_csv("../data/water_quality_oliver.csv")
```
...
> > ## Solution
> >
> > ```{r ImportFullDataSolutionEcho2, eval = FALSE}
...
>
> > ## Solution
> > ```{r PlotFullGapmindersolutions, echo = FALSE}
> >  ggplot(data = water_quality_oliver) +
> >    aes(x=DayofYear, y=Conductivity) +
> >    geom_point()
> > ```
...
```{r ImportNutrients, include=FALSE}
# required for below code chunks to work.
september_nutrients <- read_csv("../data/september_nutrients.csv")
```
...
> What argument should be provided in the below code to read in the full dataset?
>
> ```{r ImportFullDataNut, eval = FALSE}
> september_nutrients <- read_csv()
> ```
>
> > ## Solution
> >
> > ```{r ImportFullDataSolutionEcho3, eval = FALSE}
...
```{r GapViolinPoints, warning = FALSE}
ggplot(data = september_nutrients) +
  aes(x = Station, y = Chl_a) +
  geom_boxplot() +
  geom_point()
...
```{r GapViolinJitter, warning = FALSE}
ggplot(data = september_nutrients) +
  aes(x = Station, y = Chl_a) +
  geom_boxplot() +
  geom_jitter()
...
```{r GapViolinJitterLayers, warning = FALSE}
ggplot(data = september_nutrients) +
  aes(x = Station, y = Chl_a) +
  geom_jitter() +
  geom_boxplot()
...
```{r GapViolinJitter2, warning = FALSE}
ggplot(data = september_nutrients) +
  aes(x = Station, y = Chl_a) + 
  geom_boxplot() +
  geom_jitter()
...
```{r GapViolinJitterinside, warning = FALSE}
...
```{r GapViolinJitterAes, warning = FALSE}
ggplot(data = september_nutrients) +
  aes(x = Station, y = Chl_a) +
  geom_boxplot() +
...
```{r GapViolinColor, warning = FALSE}
ggplot(data = september_nutrients) +
  aes(x = Station, y = Chl_a) +
...
```{r GapViolinFillMap, warning = FALSE}
ggplot(data = september_nutrients) +
  aes(x = Station, y = Chl_a) +
...
```{r GapViolinFillMan, warning = FALSE}
ggplot(data = september_nutrients) +
  aes(x = Station, y = Chl_a) +
  geom_boxplot(aes(fill=Station)) +
...
```{r GapViolinFillBrew, warning = FALSE}
ggplot(data = september_nutrients) +
  aes(x = Station, y = Chl_a) +
  geom_boxplot(aes(fill=Station)) +
...
> > ## Solution
> > ```{r GapViolinFillSoln, warning = FALSE}
> > ggplot(data = september_nutrients) +
> >   aes(x = Station, y = Chl_a) +
...
> ```{r GapViolinAesFillMap, warning = FALSE}
>  ggplot(data = september_nutrients) +
>  aes(x = Station, y = Chl_a) +
...
```{r GapLifeHistBins, warning = FALSE}
ggplot(september_nutrients) +
  aes(x = Chl_a) +
...
```{r GapLifeHistBinsClassicTheme, warning = FALSE}
ggplot(data = september_nutrients) +
  aes(x = Station, y = Chl_a) +
  geom_boxplot() +
  theme_classic()
...
>
> > ## Example solution
> > ```{r savingPlotExercise, warning = FALSE}
> > ggplot(september_nutrients) +
> >   aes(x = Chl_a) +
...
```{r violinPlotBWThemeUpdated, warning = FALSE}
box_plot
...
>
> > ## Example solution
> > ```{r createPlotSave, warning = FALSE}
